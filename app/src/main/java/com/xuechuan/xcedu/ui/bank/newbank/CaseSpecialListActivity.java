 ...
 */
public class CaseSpecialListActivity extends BaseActivity implements View.OnClickListener, GmReadTwoFragment.notification {
        ...
        if (mDoLogProgressSqliteHelp != null) {
            if (mMainDataLists != null && !mMainDataLists.isEmpty()) {
                final DoLogProgreeSqliteVo look = mDoLogProgressSqliteHelp.findLookWithTidChapterId(
                        Integer.parseInt(mTagId), Integer.parseInt(mCourseId), DataMessageVo.ORDER_THREE);
                if (look == null) return;
